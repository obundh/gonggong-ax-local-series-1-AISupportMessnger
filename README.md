...
- DGX Spark용 `offline/` 페이로드와 Windows 설치본
...
DGX 실행 스크립트는 참고용으로 포함하지만 공개 저장소에는 `offline/` 런타임 페이로드가 없습니다. 현재 폴더만으로 원클릭 오프라인 설치는 되지 않습니다. 자세한 내용은 `README_DGX_SPARK_PACKAGE.md`를 확인하세요.
